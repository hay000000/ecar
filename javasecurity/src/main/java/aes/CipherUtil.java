...
public class CipherUtil {
	...
	public static String encrypt(String plain) {
		...
		try {
			randomKey = getRandomKey("AES"); //암호화에 사용된 키 설정. 복호화에서 사용되야함.
			Key key = new SecretKeySpec(randomKey, "AES");
			AlgorithmParameterSpec paramSpec = new IvParameterSpec(iv);
			cipher.init(Cipher.ENCRYPT_MODE, key, paramSpec);
			// AES 알고리즘. CBC모드, PKCSSPadding,
	...
	public static String decrypt(String cipherMsg) {
		...
		try {
			...
			//AES 알고리즘. CBC모드, PKCSSPadding, 복호화 모드, 키 설정, 초기화 벡터
	...
	public static void encryptFile(String plainFile, String cipherFile, String strkey) {
		 try {
			 ...
			 FileOutputStream fos = new FileOutputStream(cipherFile); //"c.sec" => 원본파일
			 //암호 관련 스트림
